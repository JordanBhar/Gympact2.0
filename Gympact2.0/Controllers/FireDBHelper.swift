    ...
    func getUserData(userData: User){
        ...
        userData.getDocument { (document, error) in
            if let document = document, document.exists {
                ...
                let User = User(uid: Auth.auth().currentUser!.uid, email: email, gender: gender, age: age, feet: feet, inches: inches, weight: weight, goal_weight: goalWeight, goal_muscle: goalMuscle)
                
                self.currentUser = User // Set the currentUser
